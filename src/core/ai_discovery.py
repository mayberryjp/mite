...
def is_ai_rate_limited():
    """Public check: True when the 24h AI call budget is exhausted."""
    return not _check_rate_limit()
...
def classify_patterns(patterns):
    ...
    if not _check_rate_limit():
        ...
        log_warn(
            logger,
            f"[WARN] AI rate limit reached: {count}/{rate_limit} calls in 24h window. Skipping classification.",
        )
        return {
            "status": "error",
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            log_warn(
                logger, f"[WARN] AI API returned HTTP {resp.status_code}: {error_body}"
            )
            return {
                "status": "error",
        ...
        for result in results:
            ...
            if classification not in VALID_CLASSIFICATIONS:
                log_warn(
                    logger,
                    f"[WARN] Invalid classification '{classification}' for pattern {pattern_id}",
                )
                continue
...
def classify_single_pattern(pattern):
    ...
    if not AI_API_BASE_URL or not AI_API_KEY:
        log_warn(
            logger,
            "[WARN] AI API not configured — cannot classify pattern. Set AI_API_BASE_URL, AI_API_KEY, and AI_MODEL",
        )
        return None
    ...
    if result.get("status") == "error":
        log_warn(
            logger,
            f"[WARN] AI classification failed for pattern {pattern['id']}: {result.get('message', 'unknown error')}",
        )
        return None
    ...
    log_warn(
        logger, f"[WARN] AI returned no classification for pattern {pattern['id']}"
    )
    return None
